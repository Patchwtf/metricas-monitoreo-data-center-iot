...
import com.metricas_monitoreo_data_center_iot.com.security.service.AutenticacionService;
import com.metricas_monitoreo_data_center_iot.com.service.RolesService;
import com.metricas_monitoreo_data_center_iot.com.security.dto.AuthRequest;
import com.metricas_monitoreo_data_center_iot.com.security.dto.AuthResponse;
import com.metricas_monitoreo_data_center_iot.com.security.dto.RegisterRequest;
import com.metricas_monitoreo_data_center_iot.com.security.jwt.JwtUtil;
...
public class AutenticationController {
    ...
    public ResponseEntity<?> login(@Valid @RequestBody AuthRequest request) {
        try {
            ...
            log.info("USUARIO: " + response.getCorreo() + " inicio sesión exitosamente.");
            return ResponseEntity.ok(response);
        } catch (BadCredentialsException e) {
            log.warn("USUARIO: " + request.getCorreo() + " error al iniciar sesión. Credenciales invalidas.");
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                    .body("Credenciales inválidas");
        } catch (LockedException e) {
            log.warn("USUARIO: " + request.getCorreo() + " error al iniciar sesión. Usuario Bloqueado.");
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                    .body("Usuario bloqueado");
        } catch (Exception e) {
            log.warn("USUARIO: " + request.getCorreo() + " error al iniciar sesión..");
    ...
    public ResponseEntity<?> register(@Valid @RequestBody RegisterRequest request) {
        try {
            if (usuarioRepository.existsByCorreo(request.getCorreo())) {
                log.warn("Error en intento de registro\t|\tEstatus: El correo ya se encuentra regostrado");
